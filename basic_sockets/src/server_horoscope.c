...
void *connection_handler(void *socket_desc)
{
    ...
    {
        printf("(Horoscope Server) ERROR: No message sent.\n");
        exit(EXIT_FAILURE);
    }

    printf("(Horoscope Server) LOG: Handling request.\n");
    sleep(8);
    ...
    printf("(Horoscope Server) LOG: Response sent.\n");

    // close connection to the client
    close(client_socket);
    printf("(Horoscope Server) LOG: Connection closed.\n");
    free(socket_desc);
    pthread_exit(NULL);
}
...
int main()
{
    ...
    {
        printf("(Horoscope Server) ERROR: enviroment vars dont setted correctly.\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        perror("(Horoscope Server) ERROR: Socket creation failed");
        exit(EXIT_FAILURE);
    }
    ...
    {
        perror("(Horoscope Server) ERROR: Bind failed");
        exit(EXIT_FAILURE);
    }
    // Listen for connections
    if (listen(server_fd, MAX_CONNECTIONS) < 0)
    {
        perror("(Horoscope Server) ERROR: Listen failed");
        exit(EXIT_FAILURE);
    }
    printf("(Horoscope Server) LOG: Listening in PORT %d.\n", atoi(getenv("PORT")));
    while (1)
    {
        // Accept the connection instance
        if ((client_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t *)&addrlen)) < 0)
        {
            perror("(Horoscope Server) ERROR: Connection failed\n");
            exit(EXIT_FAILURE);
        }
        printf("(Horoscope Server) LOG: Conneccion instance accepted.\n");
        ...
        *new_socket = client_socket;
        ...
        {
            perror("(Horoscope Server) ERROR: Thread creation failed");
            exit(EXIT_FAILURE);
        }
        printf("(Horoscope Server) LOG: Handling connection.\n");
    }
